...
import com.nokor.efinance.client.jersey.ClientSubsidy;
 ...
 *
 */
public class TestWSSubsidy extends TestCase {
    ...
    public void testGetAssetRanges() {
    	try {
    		Gson gson = new Gson();
			Response response = ClientSubsidy.getSubsidy(URL);
			logger.info("Response Status: " + response.getStatus());
			if (response.getStatus() == Status.OK.getStatusCode()) {
				List<SubsidyDTO> subsidyDTOs = response.readEntity(new GenericType<List<SubsidyDTO>>() {});
				for (SubsidyDTO subsidyDTO : subsidyDTOs) {
//					logger.info("AssetRange Code:" + subsidyDTO.getCode());
					logger.info("JSON: \r\n" + gson.toJson(subsidyDTO));
				}
				logger.info("Nb subsidy found :" + subsidyDTOs.size());
				logger.info("************SUCCESS**********");
			} else {
