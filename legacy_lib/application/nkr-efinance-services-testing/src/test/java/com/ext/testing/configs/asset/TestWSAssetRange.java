 ...
 * @author uhout.cheng
 */
public class TestWSAssetRange extends TestCase {
	...
	/**
     * Get all asset ranges
     */
    public void testGetAssetRanges() {
    	try {
    		Gson gson = new Gson();
			Response response = ClientAssetRange.getAssetRanges(URL);
			logger.info("Response Status: " + response.getStatus());
			if (response.getStatus() == Status.OK.getStatusCode()) {
				List<AssetRangeDTO> assetRangeDTOs = response.readEntity(new GenericType<List<AssetRangeDTO>>() {});
				for (AssetRangeDTO assetRangeDTO : assetRangeDTOs) {
					logger.info("AssetRange Code:" + assetRangeDTO.getCode());
					logger.info("JSON: \r\n" + gson.toJson(assetRangeDTO));
				}
				logger.info("Nb asset ranges found :" + assetRangeDTOs.size());
				logger.info("************SUCCESS**********");
			} else {
