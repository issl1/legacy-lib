...
import com.nokor.efinance.core.applicant.model.ApplicantReferenceContactInfo;
import com.nokor.efinance.core.applicant.model.ApplicantReferenceInfo;
import com.nokor.efinance.core.applicant.model.EApplicantReferenceType;
...
import com.nokor.efinance.core.common.organization.model.ERelationship;
import com.nokor.frmk.testing.BaseTestCase;

public class LesseeTest extends BaseTestCase {
	...
	public void testPersonalInformation() {
		...
		String s = "110|System ID|ID type|IDNumber|Issuing ID date|Expiring ID date|Customer ID|Prefix|First name |Last name|Nickname|DateOfBirth|Status|Children|Education|Household|Nationality|Religion|Second language|Household Expenses|Household Income";
		...
		try {
			...
			for (int i = 0; i< lines.size(); i++) {
				...
				if ("110".equals(lesseesData[0]) && !"System ID".equals(lesseesData[1])) {
			...
			logger.debug("total line not enought :" + countLineNotEnought);
	...
	public void xxtestFinancialInformation() {
		...
		try {
			applicantService = getBean(ApplicantService.class);
			file = readFile("011-1-201507211636.csv");
			List<String> lines = FileUtils.readLines(file);
			for (int i = 0; i< lines.size(); i++) {
	...
	public void xxtestAddressInformation() {
		...
		try {
			applicantService = getBean(ApplicantService.class);
			file = readFile("012-1-201507211636.csv");
			
			List<String> lines = FileUtils.readLines(file);
			for (int i=0; i<lines.size(); i++) {
				String[] addressInfoDatas = lines.get(i).split(cvsSplitBy, -1);
				if ("012".equals(addressInfoDatas[0]) && !"System ID".equals(addressInfoDatas[1])) {
	...
	public void xxtestContactInformation() {
		...
		try {
			applicantService = getBean(ApplicantService.class);
			file = readFile("013-1-201507171618.csv");
			
			List<String> lines = FileUtils.readLines(file);
			for (int i=0; i<lines.size(); i++) {
				String[] contactInfoDatas = lines.get(i).split(cvsSplitBy, -1);
				if ("013".equals(contactInfoDatas[0]) && !"System ID".equals(contactInfoDatas[1])) {
	...
	public void xxtestReferenceInfomation() {
		...
		try {
			applicantService = getBean(ApplicantService.class);
			file = readFile("014-1-123456789.csv");
			
			List<String> lines = FileUtils.readLines(file);
			for (int i=0; i<lines.size(); i++) {
				String[] lesseeReferenceInfoDatas = lines.get(i).split(cvsSplitBy);
				if ("014".equals(lesseeReferenceInfoDatas[0]) && !"System ID".equals(lesseeReferenceInfoDatas[1])) {
					if (length == lesseeReferenceInfoDatas.length) {

						Applicant lessee = getApplicantByMigrationID(lesseeReferenceInfoDatas[1]);
						if (lessee == null) {
							lessee = new Applicant();
							lessee.setMigrationID(lesseeReferenceInfoDatas[1]);
							applicantService.saveOrUpdate(lessee);
						}
						ApplicantReferenceInfo applicantReferenceInfo = new ApplicantReferenceInfo();
						applicantReferenceInfo.setApplicant(lessee);
						applicantReferenceInfo.setReferenceType(EApplicantReferenceType.getByCode(lesseeReferenceInfoDatas[2]));
						applicantReferenceInfo.setRelationship(ERelationship.getByCode(lesseeReferenceInfoDatas[3]));
						applicantReferenceInfo.setFirstNameEn(lesseeReferenceInfoDatas[4]);
						applicantReferenceInfo.setLastNameEn(lesseeReferenceInfoDatas[5]);
						applicantService.saveOrUpdate(applicantReferenceInfo);
						
						ContactInfo contactInfo = new ContactInfo();
						contactInfo.setTypeInfo(ETypeContactInfo.getByCode(lesseeReferenceInfoDatas[6]));
						contactInfo.setTypeAddress(ETypeAddress.getByCode(lesseeReferenceInfoDatas[7]));
						contactInfo.setValue(lesseeReferenceInfoDatas[8]);
						applicantService.saveOrUpdate(contactInfo);
						
						ApplicantReferenceContactInfo applicantReferenceContactInfo = new ApplicantReferenceContactInfo();
						applicantReferenceContactInfo.setApplicantReferenceInfo(applicantReferenceInfo);
						applicantReferenceContactInfo.setContactInfo(contactInfo);
						applicantService.saveOrUpdate(applicantReferenceContactInfo);
					}
