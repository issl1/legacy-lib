 ...
 * @author ly.youhort
 */
public class GLFInterestIncomeCalculatorImpl {
		
	Logger logger = LoggerFactory.getLogger(GLFInterestIncomeCalculatorImpl.class);
	...
	public InterestIncomeSchedules getSchedules(Date startDate, Date firstInstallmentDate, CalculationParameter calculationParameter, List<Cashflow> calculatedCashflows, Date searchStartDate, Date searchEndDate) {
		...
		for (int i = 0; i <= numberOfPeriods; i++) {
			...
			for (Cashflow interestCashflow : interestRepaymentCashflows) {
				...
				if (interestCashflow.getNumInstallment() == 1) {
					revenueRepayment += revenueSchedules.getSchedules().get(0).getRevenueAmount();
					if (i != interestCashflow.getNumInstallment()) {
						currentRevenueRepayment += revenueSchedules.getSchedules().get(0).getRevenueAmount();
					}
				}
				revenueRepayment += revenueSchedules.getSchedules().get(interestCashflow.getNumInstallment()).getRevenueAmount();
				if (i != interestCashflow.getNumInstallment() || (contractInOverdueMoreThanOneMonth && i == interestCashflow.getNumInstallment())) {
					if (contractInOverdueMoreThanOneMonth) {
						currentRevenueRepayment += revenueSchedules.getSchedules().get(interestCashflow.getNumInstallment()).getRevenueAmount();
					}
				}
