...
/**
 * Monthly payment schedule report for template_2
 * @author sok.vina
 */
public class GLFPaymentScheduleKH2 extends XLSAbstractReportExtractor implements GLFPaymentScheduleFields {
	...
	public String generate(ReportParameter reportParameter) throws Exception {
		...
		insuranceFee = getServiceFee("INSFEE", quotation.getQuotationServices()); 
		servicingFee = getServiceFee("SERFEE", quotation.getQuotationServices());
		...
		iCol = 7;
		createPriceGeneral(rowModel, iCol, quotation.getTiFinanceAmount(), true, false, true, CellStyle.ALIGN_CENTER, 11, false, 2, true, BG_WHITE, FC_GREEN);
		iCol = 8;
		...
		iCol = 7;
		createPercentageValueCell(rowYear, iCol, quotation.getInterestRate() / 100, false, true, CellStyle.ALIGN_CENTER, 11, 2, true, true, BG_WHITE, FC_BLUE);
		...
		iCol = 7;
		createNumericCell(rowCc, iCol, quotation.getTerm(), true, CellStyle.ALIGN_CENTER, 11, false, true, BG_WHITE, FC_BLUE);
		iCol = 8;
		...
		iCol = 7;
		createPriceGeneral(rowVehCashPice, iCol, quotation.getTiInstallmentAmount(), true, false, true, CellStyle.ALIGN_CENTER, 11, false, 2, true, BG_WHITE, FC_GREEN);
		iCol = 8;
		...
		iCol = 3;
		createPercentageValueCell(rowAdvancePaymentRate, iCol, quotation.getAdvancePaymentPercentage() / 100, false, true, CellStyle.ALIGN_CENTER, 11, 2, true, true, BG_WHITE, FC_BLUE);
		...
		iCol = 7;
		createDateCell(rowAdvancePaymentRate, iCol, contractStartDate, true, CellStyle.ALIGN_CENTER, 11, false, true, BG_WHITE, FC_BLUE);
		iCol = 8;
		...
		iCol = 3;
		createPriceGeneral(rowAdvancePayment, iCol, MyNumberUtils.getDouble(quotation.getTiAdvancePaymentAmount()), true, false, true, CellStyle.ALIGN_CENTER, 11, false, 2, true, BG_WHITE, FC_GREEN);
		...
		iCol = 7;
		createDateCell(rowAdvancePayment, iCol, firstPaymentDate, true, CellStyle.ALIGN_CENTER, 11, false, true, BG_WHITE, FC_BLUE);
		iCol = 8;
	...
	private int paymentTable(final Sheet sheet, int iRow, final CellStyle style, List<Schedule> schedules) throws Exception {
		...
		for (int i = 0; i < nbRows; i++) {
			...
			if (i < schedules.size()) {
				...
				createPriceGeneral(tmpRow, iCol++, schedule.getInstallmentPayment() + servicingFee + insuranceFee , true, false, false, CellStyle.ALIGN_RIGHT, 12, false, 2, true, BG_WHITE, FC_BLACK);
				createPriceGeneral(tmpRow, iCol++, servicingFee, true, false, false, CellStyle.ALIGN_RIGHT, 12, false, 2, true, BG_WHITE, FC_BLACK);
				createPriceGeneral(tmpRow, iCol++, insuranceFee, true, false, false, CellStyle.ALIGN_RIGHT, 12, false, 2, true, BG_WHITE, FC_BLACK);
