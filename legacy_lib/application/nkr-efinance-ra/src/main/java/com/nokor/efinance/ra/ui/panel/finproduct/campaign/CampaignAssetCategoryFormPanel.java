...
import com.nokor.efinance.core.financial.service.CampaignAssetCategoryRestriction;
...
/**
 * Campaign Asset category Form Panel
 * @author bunlong.taing
 */
public class CampaignAssetCategoryFormPanel extends AbstractTabPanel implements ClickListener {
	/** */
	...
	public CampaignAssetCategoryFormPanel() {
		super();
	}
	...
	private List<ColumnDefinition> createColumnDefinition() {
		...
		columnDefinitions.add(new ColumnDefinition(CampaignAssetCategory.ID, I18N.message("id"), Long.class, Align.LEFT, 70));
		columnDefinitions.add(new ColumnDefinition(CampaignAssetCategory.ASSETCATEGORY + "." + AssetCategory.DESCEN, I18N.message("name"), String.class, Align.LEFT, 250));
		return columnDefinitions;
	}
	
	/**
	 * @param campaignAssetCategories
	 */
	@SuppressWarnings("unchecked")
	private void setTableDataSource(List<CampaignAssetCategory> campaignAssetCategories) {
		Container container = pagedTable.getContainerDataSource();
		if (campaignAssetCategories != null) {
			for (CampaignAssetCategory campaignAssetCategory : campaignAssetCategories) {
				Item item = container.addItem(campaignAssetCategory.getId());
				item.getItemProperty(CampaignAssetCategory.ID).setValue(campaignAssetCategory.getId());
				AssetCategory assetCategory = campaignAssetCategory.getAssetCategory();
				if (assetCategory != null) {
					item.getItemProperty(CampaignAssetCategory.ASSETCATEGORY + "." + AssetCategory.DESCEN).setValue(assetCategory.getDescLocale());
				}
			}
		}
	...
	public void assignValues(Long campaignId) {
		...
		if (campaignId != null) {
			...
			CampaignAssetCategoryRestriction restriction = new CampaignAssetCategoryRestriction();
			...
			dialog.show(new AssetCategorySelectPanel.Listener() {
				...
				public void onClose(AssetCategorySelectPanel dialog) {
					...
					if (ids != null && !ids.isEmpty()) {
						List<CampaignAssetCategory> campaignAssetCategories = new ArrayList<>();
						Campaign campaign = ENTITY_SRV.getById(Campaign.class, campaignId);
						for (Long id : ids) {
							CampaignAssetCategory campaignAssetCategory = new CampaignAssetCategory();
							campaignAssetCategory.setAssetCategory(ENTITY_SRV.getById(AssetCategory.class, id));
							campaignAssetCategory.setCampaign(campaign);
							campaignAssetCategories.add(campaignAssetCategory);
						}
						ENTITY_SRV.saveOrUpdateBulk(campaignAssetCategories);
