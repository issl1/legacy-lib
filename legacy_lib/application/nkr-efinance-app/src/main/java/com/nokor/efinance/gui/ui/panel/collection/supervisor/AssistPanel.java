 ...
 *
 */
public class AssistPanel extends VerticalLayout implements FMEntityField, FinServicesHelper {
	...
	private EColType assistTeam;
	//private String profileCode;
	
	private VerticalLayout messagePanel;
	
	public AssistPanel() {
		setCaption(I18N.message("assist"));
		setMargin(true);
		setSpacing(true);
	    ...
	    pagedTable.addItemClickListener(new ItemClickListener() {
			...
			public void itemClick(ItemClickEvent event) {
				...
				if (event.isDoubleClick()) {
					...
					assistPopupWindow(ENTITY_SRV.getById(Contract.class, contractId));
				}
				
			}
		});
	    
	    setAssistTeamAndProfileCode();
	...
	private IndexedContainer getIndexedContainer() {
		...
		int index = 0;
		List<Contract> contracts = COL_SRV.getAssistContractStatusApporve(assistTeam);
	 ...
	 * @param contract
	 */
	private void assistPopupWindow(Contract contract) {
		...
		cbxStaff = new SecUserComboBox(I18N.message("staff"), COL_SRV.getCollectionUsers(new String[]{IProfileCode.COL_PHO_STAFF}));
		...
		btnSave.addClickListener(new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if (checkRequire()) {
					...
					COL_SRV.assignAssistRequest(contract.getId());
					window.close();
	 ...
	 */
	private void setAssistTeamAndProfileCode() {
		//profileCode = "";
		if (ProfileUtil.isColPhoneSupervisor()) {
			assistTeam = EColType.PHONE;
			//profileCode = "COL_PHO_SUPER";
		} else if (ProfileUtil.isColFieldSupervisor()) {
			assistTeam = EColType.FIELD;
			//profileCode = "COL_FIE_SUPER";
		} else if (ProfileUtil.isColInsideRepoSupervisor()) {
			assistTeam = EColType.INSIDE_REPO;
			//profileCode = "COL_INS_SUPER";
		} else if (ProfileUtil.isColOASupervisor()) {
			assistTeam = EColType.OA;
			//profileCode = "COL_OA_SUPER";
		}
	}
