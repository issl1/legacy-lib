...
public class PurchaseOrderReportTablePanel extends AbstractTablePanel<Payment> implements PaymentEntityField {
	...
	protected PagedDataProvider<Payment> createPagedDataProvider() {
		...
		pagedDefinition.setRowRenderer(new PurchaseOrderRowRenderer());
		...
		pagedDefinition.addColumnDefinition("purchase.order.date", I18N.message("purchase.order.date"), Date.class, Align.RIGHT, 100);
	...
	@Override
	protected PurchaseOrderReportSearchPanel createSearchPanel() {
		return new PurchaseOrderReportSearchPanel(this);		
	}
		
	/**
	 * @author ly.youhort
	 */
	private class PurchaseOrderRowRenderer implements RowRenderer, PaymentEntityField {
		...
		public void renderer(Item item, Entity entity) {
			...
			item.getItemProperty("total.payment").setValue(AmountUtils.convertToAmount(
					MyNumberUtils.getDouble(contract.getTiAdvancePaymentAmount()) + tiInsuranceFeeUsd + tiServicingFeeUsd + tiRegistrationFeeUsd + tiOtherUsd));
		}
	}
