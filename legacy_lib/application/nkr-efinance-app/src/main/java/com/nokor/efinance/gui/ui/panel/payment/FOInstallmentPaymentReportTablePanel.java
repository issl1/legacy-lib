...
public class FOInstallmentPaymentReportTablePanel extends AbstractTablePanel<Payment> implements PaymentEntityField {
	...
	protected PagedDataProvider<Payment> createPagedDataProvider() {
		...
		pagedDefinition.addColumnDefinition("installment.payment.no", I18N.message("installment.payment.no"), String.class, Align.LEFT, 140);
		...
		pagedDefinition.addColumnDefinition(DUE_DATE, I18N.message("due.date"), Date.class, Align.LEFT, 110);
		pagedDefinition.addColumnDefinition(PAYMENT_DATE, I18N.message("payment.date"), Date.class, Align.LEFT, 110);
		pagedDefinition.addColumnDefinition(PAYMENT_METHOD, I18N.message("payment.method"), String.class, Align.LEFT, 110);
	...
	private class PaymentRowRenderer implements RowRenderer, PaymentEntityField {
		...
		public void renderer(Item item, Entity entity) {
			...
			if (cashflows != null && !cashflows.isEmpty()) {
				...
				item.getItemProperty("contract.date").setValue(contract != null ? contract.getSigatureDate() : null);
				item.getItemProperty("contract").setValue(contract != null ? contract.getReference() : "");
				...
				item.getItemProperty("motor.model").setValue(contract != null ? contract.getAsset().getDescEn() : "");
