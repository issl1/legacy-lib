package com.nokor.efinance.gui.ui.panel.collection.transfer;
...
/**
 * Transfer tab panel in collection
 * @author uhout.cheng
 */
@Component
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
public class TransferPanel extends AbstractTabPanel {
	/** */
	private static final long serialVersionUID = 95712067584134693L;
	
	private static String AGREE = I18N.message("agree.to.by.pass.to");
	private static String REJECT = I18N.message("reject");
	...
	public TransferPanel() {
		super();
		setCaption(I18N.message("transfer"));
		setSizeFull();
	}
	...
	protected com.vaadin.ui.Component createForm() {
		...
		cbxSuggest = new ComboBox();
		
		btnTransfer = new Button(I18N.message("transfer"));
		btnTransfer.addClickListener(new ClickListener() {

			/** */
			private static final long serialVersionUID = -6857634853732103365L;
	    ...
	    pagedTable.addItemClickListener(new ItemClickListener() {
			...
			/** */
			private static final long serialVersionUID = -724672093232080664L;
			...
			public void itemClick(ItemClickEvent event) {
				if (event.isDoubleClick()) {
					...
					formPanel.setCaption(I18N.message("contract"));
	    ...
	    contentLayout.addComponent(btnTransfer);
	    contentLayout.setComponentAlignment(btnTransfer, Alignment.BOTTOM_RIGHT);
	...
	/**
	 * Pop up transfer window
	 */
	public void createTransferWindow(){
		final Window winTransfer = new Window(I18N.message("tansfer.job.or.return.contract"));
		winTransfer.setModal(true);
		winTransfer.setWidth(380, Unit.PIXELS);
		winTransfer.setHeight(270, Unit.PIXELS);
		Label lblSuggest = new Label("Suggest to by pass to: Field");
		
		Panel panel = new Panel();
		panel.setSizeUndefined();
		panel.setHeight("50px");
		panel.setContent(lblSuggest);
		
		//Create Button Transfer and set Listener
        Button btnTransfer = new NativeButton(I18N.message("transfer"), new Button.ClickListener() {

        	/** */
			private static final long serialVersionUID = 3372557754251106202L;
        ...
        btnTransfer.setIcon(new ThemeResource("../nkr-default/icons/16/tick.png"));
        //Create Button Cancel and set Listener to Button
        Button btnCancel = new NativeButton(I18N.message("cancel"), new ClickListener() {

			/** */
			private static final long serialVersionUID = 6067005844835019258L;
        ...
        final List<String> options = Arrays.asList(new String[] {AGREE, REJECT});
        ...
        //Create NavigationPanel for set Button Transfer & button cancel
        NavigationPanel navigationPanel = new NavigationPanel();
		navigationPanel.addButton(btnTransfer);
		...
		gridLayout.addComponent(optionGroup, 0 ,1);
		gridLayout.addComponent(cbxSuggest, 1 ,1);
		verticalLayout.addComponent(gridLayout);

		winTransfer.setContent(verticalLayout);
        UI.getCurrent().addWindow(winTransfer);
	}
}
