...
/**
 * Report on the Ratio of Application to Contract
 * @author bunlong.taing
 *
 */
@Component
@Scope(BeanDefinition.SCOPE_PROTOTYPE)
@VaadinView(RatioNumApplicationToNumContratReportPanel.NAME)
public class RatioNumApplicationToNumContratReportPanel extends AbstractTabPanel implements View, QuotationEntityField, FrmkServicesHelper {

	/** */
	private static final long serialVersionUID = -8133595112515308208L;
	public static final String NAME = "ratio.num.app.to.num.con";
	...
	protected com.vaadin.ui.Component createForm() {
		...
		cbxDealerType.addValueChangeListener(new ValueChangeListener() {
			/** */
			private static final long serialVersionUID = -528468395763821051L;
			@Override
		...
		tableRatio.setCaption(I18N.message("numbers.applicant.and.contract"));
		...
		tabSheet.addTab(contentLayout, I18N.message("numbers.applicant.and.contract"));
	...
	private List<ColumnDefinition> createColumnDefinitions() {
		...
		columnDefinitions.add(new ColumnDefinition(RATIO, I18N.message("ratio.application.contract"), String.class, Align.LEFT, 200));
	...
	private Button.ClickListener getButtonClickListener() {
		return new Button.ClickListener() {
			/** */
			private static final long serialVersionUID = 6447999633236580791L;
			@Override
	...
	private void onSearch () {
		...
		for (int i = 0; i < numberOfMonth; i++) {
			...
			long numApplicant = getNbApplicationsFromBeginOfMonth(searchDate, dealer, dt);
			long numContract = getNbContractsFromBeginOfMonth(searchDate, dealer, dt);
			...
			double ratio = numApplicant == 0 ? 0 : ((double) numContract / numApplicant) * 100;
	...
	/**
	 * Get numbers of application from the beginning of the month
	 * @param date The month to be searched
	 * @param dealer The dealer to be searched
	 * @return Number of application
	 */
	private long getNbApplicationsFromBeginOfMonth(Date date, Dealer dealer, EDealerType dt) {
	...
	/**
	 * Get the number of total contract in the month of the date
	 * provided with the provided dealer
	 * @param date The date to be searched
	 * @param dealer The dealer to be searched
	 * @return The number of contracts
	 */
	private long getNbContractsFromBeginOfMonth (Date date, Dealer dealer, EDealerType dt) {
		...
		if (dealer != null) {
			restrictions.addCriterion(Restrictions.eq("condeal.id", dealer.getId()));
		}
		if (dt != null) {
			restrictions.addCriterion(Restrictions.eq("condeal.dealerType", dt));
		}
		List<Contract> contracts = ENTITY_SRV.list(restrictions);
		if (contracts != null) return contracts.size();
		return  0L;
	}
