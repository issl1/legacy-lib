...
@VaadinView(ServicingIncomeAdjustmentsReportPanel.NAME)
public class ServicingIncomeAdjustmentsReportPanel extends AbstractTabPanel implements View, ContractEntityField, FrmkServicesHelper {
	
	private static final long serialVersionUID = 8019051395954081483L;

	public static final String NAME = "servicing.income.adjustments.report";
	...
	private SimplePagedTable<ServicingIncomeAdjustment> pagedTable;
	...
	public ServicingIncomeAdjustmentsReportPanel() {
		super();
		setSizeFull();
	}
	...
	protected com.vaadin.ui.Component createForm() {
		...
		cbxDealer.setFilteringMode(FilteringMode.CONTAINS);
        ...
        tabSheet.addTab(contentLayout, I18N.message("service.income.adjustments"));
	...
	private void search() {
		if (isValid()) {
			List<ServicingIncomeAdjustment> servicingIncomeAdjustments = accountingService
					.getServicingIncomeAdjustments(
			...
			setIndexedContainer(servicingIncomeAdjustments);
		}
	}
	...
	private void setIndexedContainer(List<ServicingIncomeAdjustment> servicingIncomeAdjustments) {
		Indexed indexedContainer = pagedTable.getContainerDataSource();
		indexedContainer.removeAllItems();
		double totalBalanceServicingIncomeInSuspend = 0d;
		double totalUnpaidUnearnedServicingIncome = 0d;
		double totalUnpaidAccrualReceivable = 0d;
		
		for (ServicingIncomeAdjustment servicingIncomeAdjustment : servicingIncomeAdjustments) {
			Item item = indexedContainer.addItem(servicingIncomeAdjustment.getId());
			item.getItemProperty(REFERENCE).setValue(servicingIncomeAdjustment.getReference());
			item.getItemProperty(CONTRACT_STATUS).setValue(servicingIncomeAdjustment.getWkfStatus().getDesc());
			item.getItemProperty(START_DATE).setValue(servicingIncomeAdjustment.getContractStartDate());
			item.getItemProperty("firstInstallmentDate").setValue(servicingIncomeAdjustment.getFirstInstallmentDate());
			item.getItemProperty(LAST_NAME_EN).setValue(servicingIncomeAdjustment.getLastNameEn());
			item.getItemProperty(FIRST_NAME_EN).setValue(servicingIncomeAdjustment.getFirstNameEn());
			item.getItemProperty("balanceInsuranceIncomeInSuspend").setValue(servicingIncomeAdjustment.getBalanceServicingIncomeInSuspend());
			item.getItemProperty("unpaidUnearnedServicingIncome").setValue(servicingIncomeAdjustment.getUnpaidUnearnedServicingIncome());
			item.getItemProperty("unpaidAccrualReceivable").setValue(servicingIncomeAdjustment.getUnpaidAccrualReceivable());
			
			totalBalanceServicingIncomeInSuspend += MyNumberUtils.getDouble(servicingIncomeAdjustment.getBalanceServicingIncomeInSuspend().getTiAmount());
			totalUnpaidAccrualReceivable += MyNumberUtils.getDouble(servicingIncomeAdjustment.getUnpaidAccrualReceivable().getTiAmount());
			totalUnpaidUnearnedServicingIncome += MyNumberUtils.getDouble(servicingIncomeAdjustment.getUnpaidUnearnedServicingIncome().getTiAmount());
		}
		
		pagedTable.setColumnFooter("balanceInsuranceIncomeInSuspend", AmountUtils.format(totalBalanceServicingIncomeInSuspend));
		pagedTable.setColumnFooter("unpaidUnearnedServicingIncome", AmountUtils.format(totalUnpaidUnearnedServicingIncome));
		pagedTable.setColumnFooter("unpaidAccrualReceivable", AmountUtils.format(totalUnpaidAccrualReceivable));
	...
	protected List<ColumnDefinition> createColumnDefinitions() {
		...
		columnDefinitions.add(new ColumnDefinition("balanceInsuranceIncomeInSuspend", I18N.message("balance.servicing.income.in.suspend"), Amount.class, Align.RIGHT, 100));
		columnDefinitions.add(new ColumnDefinition("unpaidUnearnedServicingIncome", I18N.message("unpaid.unearned.servicing.income"), Amount.class, Align.RIGHT, 100));
		columnDefinitions.add(new ColumnDefinition("unpaidAccrualReceivable", I18N.message("unpaid.accrual.receivable"), Amount.class, Align.RIGHT, 100));
