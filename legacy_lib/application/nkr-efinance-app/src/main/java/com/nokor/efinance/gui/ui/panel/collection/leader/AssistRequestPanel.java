 ...
 *
 */
public class AssistRequestPanel extends VerticalLayout implements FMEntityField, FinServicesHelper {
	...
	public AssistRequestPanel() {
		setCaption(I18N.message("assist.request"));
		setSizeFull();
		setHeight("100%");
		setMargin(true);
		setSpacing(true);
	    ...
	    pagedTable.addItemClickListener(new ItemClickListener() {
			...
			public void itemClick(ItemClickEvent event) {
				...
				if (event.isDoubleClick()) {
					...
					requestAssistPopupWindow(ENTITY_SRV.getById(Contract.class, contractId));
				}
	...
	private IndexedContainer getIndexedContainer() {
		...
		int index = 0;
		List<Contract> contracts = COL_SRV.getRequestAssistContracts();
	 ...
	 * @param contract
	 */
	private void requestAssistPopupWindow(Contract contract) {
		final Window window = new Window(I18N.message("request.assist"));
		...
		btnValidate.addClickListener(new ClickListener() {			
			private static final long serialVersionUID = -1595776844823362727L;
			@Override
	...
	public boolean checkRequire() {
		...
		if (cbxColType.getSelectedEntity() == null) {
			errors.add(I18N.message("field.required.1",new String[] { I18N.message("assist.team") }));
		}	
