...
public class LossAdjustmentReportTablePanel extends AbstractTablePanel<Contract> implements ContractEntityField {
	...
	public void PostConstruct() {
		...
		setCaption(I18N.message("loss.adjustments"));
		setSizeFull();
		setMargin(true);
		setSpacing(true);
				
		super.init(I18N.message("loss.adjustments"));
	...
	protected PagedDataProvider<Contract> createPagedDataProvider() {
		...
		pagedDefinition.addColumnDefinition("principal.adjustment.amount", I18N.message("principal.adjustment.amount"), Amount.class, Align.RIGHT, 120);
		pagedDefinition.addColumnDefinition("interest.adjustment.amount", I18N.message("interest.adjustment.amount"), Amount.class, Align.RIGHT, 120);
	...
	@Override
	protected LossAdjustmentReportSearchPanel createSearchPanel() {
		return new LossAdjustmentReportSearchPanel(this);		
	}
	...
	private class ContractRowRenderer implements RowRenderer, ContractEntityField {
		...
		public void renderer(Item item, Entity entity) {
			...
			item.getItemProperty("principal.adjustment.amount").setValue(AmountUtils.convertToAmount(contract.getContractAdjustment().getTiAdjustmentPrincipal()));
			item.getItemProperty("interest.adjustment.amount").setValue(AmountUtils.convertToAmount(contract.getContractAdjustment().getTiAdjustmentInterest()));
		}
	}
