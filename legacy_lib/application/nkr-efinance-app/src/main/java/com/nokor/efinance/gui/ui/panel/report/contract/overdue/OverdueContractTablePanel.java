...
public class OverdueContractTablePanel extends AbstractTablePanel<Contract>
		implements FMEntityField, CashflowEntityField {
	...
	protected PagedDataProvider<Contract> createPagedDataProvider() {
		PagedDefinition<Contract> pagedDefinition = new PagedDefinition<Contract>(
				searchPanel.getRestrictions());
		...
		pagedDefinition.addColumnDefinition("province",I18N.message("province"), String.class, Align.LEFT, 150);
		pagedDefinition.addColumnDefinition("district",I18N.message("district"), String.class, Align.LEFT, 150);
	...
	private class ContractOverdueRowRenderer implements RowRenderer {
		...
		/**
		 * @see com.nokor.frmk.vaadin.ui.widget.table.RowRenderer#renderer(com.vaadin.data.Item,
		 *      org.seuksa.frmk.model.entity.Entity)
		 */
...
//
//			if (contractCollectionHistory != null) {long processingTime = DateUtils.today().getTime()- contractCollectionHistory.getCreateDate().getTime();
...
//			item.getItemProperty("collection.status").setValue(
//					contract.getCollectionStatus() != null ? contract.getCollectionStatus().getDescEn() : "");
//
...
//				item.getItemProperty("province").setValue(applicant.getApplicantAddresses().get(0).getAddress().getProvince().getDescEn());
//			}
//
//			if (quotation.getAsset() != null) {
//
//				item.getItemProperty("district").setValue(applicant.getApplicantAddresses().get(0).getAddress().getDistrict().getDescEn());
//			}
	 ...
	 * @return
	 */
	private ContractCollectionHistory getLatestContractCollectionHistory(
			List<ContractCollectionHistory> histories) {
