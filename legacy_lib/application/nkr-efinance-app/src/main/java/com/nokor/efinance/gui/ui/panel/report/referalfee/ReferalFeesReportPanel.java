...
@VaadinView(ReferalFeesReportPanel.NAME)
public class ReferalFeesReportPanel extends AbstractTabPanel implements View, ContractEntityField, FrmkServicesHelper {
	
	private static final long serialVersionUID = 8019051395954081483L;

	public static final String NAME = "referal.fees.report";
	...
	protected com.vaadin.ui.Component createForm() {
        ...
        tabSheet.addTab(contentLayout, I18N.message("Lease Transactions"));
	...
	private void search() {
		if (isValid()) {
			List<ReferalFee> referalFees = accountingService.getReferalFees(
					cbxDealerType.getSelectedEntity(),
					cbxDealer.getSelectedEntity(),
					txtContractReference.getValue(), dfStartDate.getValue(),
					dfEndDate.getValue());
			setIndexedContainer(referalFees);
		}
	}
	...
	private void setIndexedContainer(List<ReferalFee> referalFees) {
		Indexed indexedContainer = pagedTable.getContainerDataSource();
		indexedContainer.removeAllItems();
		double totalRealReferalFeeDistributed = 0d;
		double totalDeferredCommissionReferalFee = 0d;
		double totalAcrrualExpenses = 0d;
		double totalPaymentToDealer = 0d;
		
		for (ReferalFee referalFee : referalFees) {
			Item item = indexedContainer.addItem(referalFee.getId());
			item.getItemProperty(REFERENCE).setValue(referalFee.getReference());
			item.getItemProperty(START_DATE).setValue(referalFee.getContractStartDate());
			item.getItemProperty("firstInstallmentDate").setValue(referalFee.getFirstInstallmentDate());
			item.getItemProperty(LAST_NAME_EN).setValue(referalFee.getLastNameEn());
			item.getItemProperty(FIRST_NAME_EN).setValue(referalFee.getFirstNameEn());
			item.getItemProperty("realReferalFeeDistributed").setValue(referalFee.getRealReferalFeeDistributed());
			item.getItemProperty("deferredCommissionReferalFee").setValue(referalFee.getDeferredCommissionReferalFee());
			item.getItemProperty("acrrualExpenses").setValue(referalFee.getAcrrualExpenses());
			item.getItemProperty("paymentToDealer").setValue(referalFee.getPaymentToDealer());
			
			totalRealReferalFeeDistributed += MyNumberUtils.getDouble(referalFee.getRealReferalFeeDistributed().getTiAmount());
			totalDeferredCommissionReferalFee += MyNumberUtils.getDouble(referalFee.getDeferredCommissionReferalFee().getTiAmount());
			totalAcrrualExpenses += MyNumberUtils.getDouble(referalFee.getAcrrualExpenses().getTiAmount());
			totalPaymentToDealer += MyNumberUtils.getDouble(referalFee.getPaymentToDealer().getTiAmount());
		}
		
		pagedTable.setColumnFooter("realReferalFeeDistributed", AmountUtils.format(totalRealReferalFeeDistributed));
		pagedTable.setColumnFooter("deferredCommissionReferalFee", AmountUtils.format(totalDeferredCommissionReferalFee));
		pagedTable.setColumnFooter("acrrualExpenses", AmountUtils.format(totalAcrrualExpenses));
		pagedTable.setColumnFooter("paymentToDealer", AmountUtils.format(totalPaymentToDealer));
	...
	protected List<ColumnDefinition> createColumnDefinitions() {
		...
		columnDefinitions.add(new ColumnDefinition("realReferalFeeDistributed", I18N.message("real.referal.fee.distributed"), Amount.class, Align.RIGHT, 100));
		columnDefinitions.add(new ColumnDefinition("deferredCommissionReferalFee", I18N.message("deferred.commission.referal.fee"), Amount.class, Align.RIGHT, 100));
		columnDefinitions.add(new ColumnDefinition("acrrualExpenses", I18N.message("acrrual.expenses"), Amount.class, Align.RIGHT, 100));
		columnDefinitions.add(new ColumnDefinition("paymentToDealer", I18N.message("payment.to.dealer"), Amount.class, Align.RIGHT, 100));
