...
@VaadinView(InsuranceIncomeAdjustmentsReportPanel.NAME)
public class InsuranceIncomeAdjustmentsReportPanel extends AbstractTabPanel implements View, ContractEntityField, FrmkServicesHelper {
	
	private static final long serialVersionUID = 8019051395954081483L;

	public static final String NAME = "insurance.income.adjustments.report";
	...
	private SimplePagedTable<InsuranceIncomeAdjustment> pagedTable;
	...
	public InsuranceIncomeAdjustmentsReportPanel() {
		super();
		setSizeFull();
	}
	...
	protected com.vaadin.ui.Component createForm() {
        ...
        tabSheet.addTab(contentLayout, I18N.message("insurance.income.adjustments"));
	...
	private void search() {
		if (isValid()) {
			List<InsuranceIncomeAdjustment> insuranceIncomeAdjustments = accountingService
					.getInsuranceIncomeAdjustments(
			...
			setIndexedContainer(insuranceIncomeAdjustments);
		}
	}
	...
	private void setIndexedContainer(List<InsuranceIncomeAdjustment> insuranceIncomeAdjustments) {
		Indexed indexedContainer = pagedTable.getContainerDataSource();
		indexedContainer.removeAllItems();
		double totalBalanceInsuranceIncomeInSuspend = 0d;
		double totalUnpaidUnearnedInsuranceIncome = 0d;
		double totalUnpaidAccrualReceivable = 0d;
		
		for (InsuranceIncomeAdjustment insuranceIncomeAdjustment : insuranceIncomeAdjustments) {
			Item item = indexedContainer.addItem(insuranceIncomeAdjustment.getId());
			item.getItemProperty(REFERENCE).setValue(insuranceIncomeAdjustment.getReference());
			item.getItemProperty(CONTRACT_STATUS).setValue(insuranceIncomeAdjustment.getWkfStatus().getDesc());
			item.getItemProperty(START_DATE).setValue(insuranceIncomeAdjustment.getContractStartDate());
			item.getItemProperty("firstInstallmentDate").setValue(insuranceIncomeAdjustment.getFirstInstallmentDate());
			item.getItemProperty(LAST_NAME_EN).setValue(insuranceIncomeAdjustment.getLastNameEn());
			item.getItemProperty(FIRST_NAME_EN).setValue(insuranceIncomeAdjustment.getFirstNameEn());
			item.getItemProperty("balanceInsuranceIncomeInSuspend").setValue(insuranceIncomeAdjustment.getBalanceInsuranceIncomeInSuspend());
			item.getItemProperty("unpaidUnearnedInsuranceIncome").setValue(insuranceIncomeAdjustment.getUnpaidUnearnedInsuranceIncome());
			item.getItemProperty("unpaidAccrualReceivable").setValue(insuranceIncomeAdjustment.getUnpaidAccrualReceivable());
			
			totalBalanceInsuranceIncomeInSuspend += MyNumberUtils.getDouble(insuranceIncomeAdjustment.getBalanceInsuranceIncomeInSuspend().getTiAmount());
			totalUnpaidAccrualReceivable += MyNumberUtils.getDouble(insuranceIncomeAdjustment.getUnpaidAccrualReceivable().getTiAmount());
			totalUnpaidUnearnedInsuranceIncome += MyNumberUtils.getDouble(insuranceIncomeAdjustment.getUnpaidUnearnedInsuranceIncome().getTiAmount());
		}
		
		pagedTable.setColumnFooter("balanceInsuranceIncomeInSuspend", AmountUtils.format(totalBalanceInsuranceIncomeInSuspend));
		pagedTable.setColumnFooter("unpaidUnearnedInsuranceIncome", AmountUtils.format(totalUnpaidUnearnedInsuranceIncome));
	...
	protected List<ColumnDefinition> createColumnDefinitions() {
		...
		columnDefinitions.add(new ColumnDefinition("balanceInsuranceIncomeInSuspend", I18N.message("balance.insurance.income.in.suspend"), Amount.class, Align.RIGHT, 100));
		columnDefinitions.add(new ColumnDefinition("unpaidUnearnedInsuranceIncome", I18N.message("unpaid.unearned.insurance.income"), Amount.class, Align.RIGHT, 100));
