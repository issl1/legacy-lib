 ...
 * @author uhout.cheng
 */
public class ColAssistContractFilterPanel extends AbstractColPhoLeadReqConFilterPanel implements MCollection, ValueChangeListener {
	
	/** */
	private static final long serialVersionUID = -378620444576093312L;
	 ...
	 */
	public ColAssistContractFilterPanel(final ColPhoLeadReqContractTablePanel contractTablePanel) {
	...
	public ContractRestriction getRestrictions() {
		...
		restrictions.addAssociation(COL + DOT + LASTCOLLECTIONASSIST, ASS, JoinType.INNER_JOIN);
		
		List<Criterion> criterions = new ArrayList<>();
		if (cbPending.getValue()) {
			criterions.add(Restrictions.eq(ASS + DOT + CollectionAssist.REQUESTSTATUS, ERequestStatus.PENDING));
		}
		if (cbApprove.getValue()) {
			criterions.add(Restrictions.eq(ASS + DOT + CollectionAssist.REQUESTSTATUS, ERequestStatus.APPROVE));
		}
