 ...
 * @author uhout.cheng
 */
public class CallCenterUnassignedManualPanel extends Window implements ClickListener, FinServicesHelper {

	/** */
	private static final long serialVersionUID = 1819128559848443023L;
	...
	private CallCenterUnassignedOdmPanel mainPanel;
	 ...
	 * @param mainPanel
	 */
	public CallCenterUnassignedManualPanel(CallCenterUnassignedOdmPanel mainPanel) {
	...
	private GridLayout createTopLayout() {
		...
		cbxStaff.addValueChangeListener(new ValueChangeListener() {
			...
			/** */
			private static final long serialVersionUID = -8267738384671214748L;
			...
			public void valueChange(ValueChangeEvent event) {
				UserContractDetail userContractDetail = mainPanel.getContractsByStaff().get(cbxStaff.getSelectedEntity());
		...
		txtNbContracts.addValueChangeListener(new ValueChangeListener() {
			...
			/** */
			private static final long serialVersionUID = -8287561565677853471L;
			...
			public void valueChange(ValueChangeEvent event) {
				if (cbxStaff.getSelectedEntity() != null && StringUtils.isNotEmpty(txtNbContracts.getValue())) {
					UserContractDetail userContractDetail = mainPanel.getContractsByStaff().get(cbxStaff.getSelectedEntity());
					if (userContractDetail != null && ValidateUtil.checkIntegerField(txtNbContracts, "")) {
	...
	public void setStaffTableContainerDataSource() {
		...
		for (SecUser user : reassignStaffs.keySet()) {
			...
			btnRemove.addClickListener(new ClickListener() {
				...
				/** */
				private static final long serialVersionUID = 5608013097358952649L;
	...
	private boolean validateReassignStaff() {
		...
		if (StringUtils.isEmpty(txtNbContracts.getValue())) {
			...
			if (nbContract < 1) {
				errors.add(I18N.message("msg.error.nb.contract.reassign.less.than.one"));
				valid = false;
			} else if (totalNbContract < nbContract + nbAssignedContract) {
	...
	private void reassign() {
		ConfirmDialog confirmDialog = ConfirmDialog.show(UI.getCurrent(), I18N.message("confirm.reassign"), new ConfirmDialog.Listener() {
			...
			/** */
			private static final long serialVersionUID = 1972760881887345908L;
			...
			public void onClose(ConfirmDialog dialog) {
				if (dialog.isConfirmed()) {
					mainPanel.reassign(totalNbContract, reassignStaffs, true);
					mainPanel.setStaffTableContainerDataSource(false);
					mainPanel.calculateDetailData();
					close();
				}
			}
		});
	...
	public void assignValues(int totalNbContract, SecUser selectedStaff) {
		...
		if (totalNbContract > 0 && selectedStaff != null) {
			...
			List<SecUser> staffs = COL_SRV.getCollectionUsers(new String[] {IProfileCode.CAL_CEN_STAFF});
			if (!staffs.isEmpty()) {
